...
-- Descriere: Returnează o consultație după Id și ClinicId
--   Result sets:
--     1) Header (date generale + tab-uri rămase flat: Analize/Diagnostic/Concluzii)
--     2) Anamneză  (0 sau 1 rând)
--     3) Examen Clinic  (0 sau 1 rând)
--     4) Investigații structurate (0..N rânduri din ConsultationInvestigations)
-- ============================================================================
...
BEGIN
    ...
    -- Doar dacă header-ul a fost găsit, returnăm sub-tabelele
    IF @@ROWCOUNT = 0
    BEGIN
        -- result sets goale, ca să rămână 3 totale (Dapper expects fix number)
        SELECT TOP 0
    ...
    SELECT
        a.Motiv,
        a.IstoricMedicalPersonal,
        a.TratamentAnterior,
        a.IstoricBoalaActuala,
        a.IstoricFamilial,
        a.FactoriDeRisc,
        a.AlergiiConsultatie
    FROM dbo.ConsultationAnamnesis a
    ...
    SELECT
        ...
        e.Inaltime,
        e.TensiuneSistolica,
        e.TensiuneDiastolica,
        e.Puls,
        e.FrecventaRespiratorie,
        e.Temperatura,
        e.SpO2,
        e.Edeme,
        e.Glicemie,
        e.GanglioniLimfatici,
        e.ExamenClinic,
        e.AlteObservatiiClinice
    FROM dbo.ConsultationExam e
    WHERE e.ConsultationId = @Id;

...
export interface AddressAutocompleteProps<T extends FieldValues> {
  ...
  required?: boolean
  disabled?: boolean
  /** Context geografic (județ sau localitate deja selectat) — adăugat la query Photon pentru rezultate relevante */
  locationHint?: string
...
async function searchPhoton(query: string, locationHint?: string): Promise<AddressSuggestion[]> {
  // Adaugă contextul geografic (județ/localitate) la query pentru rezultate mai relevante
  const q = locationHint ? `${query} ${locationHint}` : query
  ...
  return (data.features as any[])
    ...
    .filter((s) => s.address)
    // Deduplică după address + postcode — aceeași stradă cu coduri poștale diferite rămâne ca intrări separate
    .filter((s, i, arr) => arr.findIndex((x) => x.displayText === s.displayText && x.postcode === s.postcode) === i)
    .slice(0, 7)
}
...
export const AddressAutocomplete = <T extends FieldValues>({
  ...
  useEffect(() => {
    ...
    searchPhoton(debouncedValue, locationHint)
      .then((results) => {
        if (cancelled) return
  ...
  }, [debouncedValue, locationHint, updateDropdownPos])

...
export const DoctorsListPage = () => {
  ...
  const handleExcelExport = useCallback(() => {
    const props: ExcelExportProperties = {
      ...
      header: {
        headerRows: 1,
        rows: [{ cells: [{ colSpan: 9, value: 'ValyanClinic — Lista Doctori', style: { bold: true, fontSize: 14 } }] }],
      },
    };
  ...
  const handlePdfExport = useCallback(() => {
    const props: PdfExportProperties = {
      ...
      header: {
        fromTop: 0, height: 130,
        contents: [{
          type: 'Text', value: 'ValyanClinic — Lista Doctori',
          style: { textBrushColor: '#2A5578', fontSize: 16 },
          position: { x: 10, y: 50 },
        }],
      },
    };
